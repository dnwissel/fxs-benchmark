plot_figure_02_b <- function(output_pdf, output_svg) {
  ...
  length_bias_bulk_ont <- detail_frame %>% filter(ont_bulk_detection != "Other") %>% ggplot(aes(x = ont_bulk_detection, y = length)) + geom_violin(linewidth = 1) + theme_big_simple() + scale_y_log10() + labs(y = "Length (nts)", x = "Detected by", title = "ONT (Bulk) misses due to bias against long transcripts\n ")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_02/panel_B.pdf"
  output_svg <- "results/figure_02/panel_B.svg"
}

plot_figure_02_b(output_pdf = output_pdf, output_svg = output_svg)
