plot_figure_02_c <- function(output_pdf, output_svg) {
  ...
  length_bias_bulk_pb <- detail_frame %>% filter(pb_bulk_detection != "Other") %>% ggplot(aes(x = pb_bulk_detection, y = length)) + geom_violin(linewidth = 1) + theme_big_simple() + scale_y_log10() + labs(y = "Length (nts)", x = "Detected by", title = "PB (Bulk) misses due to bias against short transcripts\n ")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_02/panel_C.pdf"
  output_svg <- "results/figure_02/panel_C.svg"
}

plot_figure_02_c(output_pdf = output_pdf, output_svg = output_svg)
