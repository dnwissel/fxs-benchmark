plot_figure_02_h <- function(output_pdf, output_svg) {
  ...
  pb_pseudobulk_unique <- detail_frame %>% filter(pb_pseudobulk_detection != "Other") %>% ggplot(aes(x = pb_pseudobulk_detection, y = coverage)) + geom_violin(linewidth = 1, scale = "width") + theme_big_simple() + labs(y = "Gene coverage", x = "Detected by", title = "PB (SC) detections associated with\n truncations") + scale_y_log10()
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_02/panel_H.pdf"
  output_svg <- "results/figure_02/panel_H.svg"
}

plot_figure_02_h(output_pdf = output_pdf, output_svg = output_svg)
