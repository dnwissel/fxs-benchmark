plot_figure_02_g <- function(output_pdf, output_svg) {
  ...
  ont_pseudobulk_unique <- detail_frame %>% filter(ont_pseudobulk_detection != "Other") %>% ggplot(aes(x = ont_pseudobulk_detection, y = coverage)) + geom_violin(linewidth = 1, scale = "width") + theme_big_simple() + labs(y = "Gene coverage", x = "Detected by", title = "ONT (SC) detections associated with\n truncations") + scale_y_log10()
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_02/panel_G.pdf"
  output_svg <- "results/figure_02/panel_G.svg"
}

plot_figure_02_g(output_pdf = output_pdf, output_svg = output_svg)
