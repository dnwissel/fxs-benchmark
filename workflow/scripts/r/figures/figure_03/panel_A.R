plot_figure_03_a <- function(output_pdf, output_svg) {
  ...
  memory <- plt_frame %>% ggplot(aes(x = depth, y = mem/1000, color = method, group = method)) + geom_point(stat = "summary", fun = mean, size = 4) + stat_summary(fun.y = mean, geom = "line", linewidth = 2, lty = 2) + stat_summary(fun.data = mean_sdl, geom = "pointrange", linewidth = 1) + theme_big_simple() + labs(fill = "", y = "Memory (USS GB)", x = "Subsampled reads", color = "") + scale_color_manual(values = cols) + facet_wrap(~type) + scale_y_log10()
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_03/panel_A.pdf"
  output_svg <- "results/figure_03/panel_A.svg"
}

plot_figure_03_a(output_pdf = output_pdf, output_svg = output_svg)
