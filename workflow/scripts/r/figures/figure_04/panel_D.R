plot_figure_04_d <- function(output_pdf, output_svg) {
  ...
  panel_e4 <- ggplot(tx_abs_quant_df, aes(x = Illumina_bulk, y = PB)) + geom_density_2d_filled(contour_var = "ndensity", alpha = 0.9) + stat_cor(aes(label = after_stat(r.label), group = 1), show.legend = FALSE, method = "pearson", size = 6, label.x.npc = 0.025, label.y.npc = 1, geom = "label", fill = "white") + scale_fill_viridis_d(option = "A", name = "Normalized density", ) + labs(x = "Illumina bulk transcript abundance (logTPM)", y = "PB transcript counts (logCPM)") + ggpubfigs::theme_big_simple() + 
      theme(legend.position = "bottom") + facet_wrap(~Method, nrow = 1)

  if (!exists("panel_e4")) {
    stop("Panel object not found: panel_e4", call. = FALSE)
  }

  panel_plot <- get("panel_e4")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_04/panel_D.pdf"
  output_svg <- "results/figure_04/panel_D.svg"
}

plot_figure_04_d(output_pdf = output_pdf, output_svg = output_svg)
