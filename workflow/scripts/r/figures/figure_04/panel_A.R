plot_figure_04_a <- function(output_pdf, output_svg) {
  ...
  time <- plt_frame %>% ggplot(aes(x = depth, y = time/60, color = method, group = method)) + geom_point(stat = "summary", fun = mean, size = 4) + stat_summary(fun.y = mean, geom = "line", linewidth = 2, lty = 2) + stat_summary(fun.data = mean_sdl, geom = "pointrange", linewidth = 1) + theme_big_simple() + labs(fill = "", y = "Runtime (m)", x = "Subsampled reads", color = "") + scale_color_manual(values = cols[c(1, 3, 4, 6)]) + facet_wrap(~type) + scale_y_log10()
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_04/panel_A.pdf"
  output_svg <- "results/figure_04/panel_A.svg"
}

plot_figure_04_a(output_pdf = output_pdf, output_svg = output_svg)
