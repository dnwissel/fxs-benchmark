plot_figure_s01_b <- function(output_pdf, output_svg) {
  ...
  p_vol_pb_bulk <- get_volcano_plot(pb_bulk_gene, names_bulk, gene_name_map)

  if (!exists("p_vol_pb_bulk")) {
    stop("Panel object not found: p_vol_pb_bulk", call. = FALSE)
  }

  panel_plot <- get("p_vol_pb_bulk")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S01/panel_B.pdf"
  output_svg <- "results/figure_S01/panel_B.svg"
}

plot_figure_s01_b(output_pdf = output_pdf, output_svg = output_svg)
