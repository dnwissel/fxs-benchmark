plot_figure_s01_c <- function(output_pdf, output_svg) {
  ...
  pb_sc_gene <- c("results/format_quantify_subsampled/gencode/pb/1_60000000.0/bambu/pseudobulk/gene_counts_formatted.tsv", "results/format_quantify_subsampled/gencode/pb/1_60000000.0/isosceles/pseudobulk/gene_counts_formatted.tsv", "results/format_quantify_subsampled/gencode/pb/1_60000000.0/kallisto/pseudobulk/gene_counts_formatted.tsv")
  ...
  p_vol_pb_sc <- get_volcano_plot(pb_sc_gene, names_sc, gene_name_map)

  if (!exists("p_vol_pb_sc")) {
    stop("Panel object not found: p_vol_pb_sc", call. = FALSE)
  }

  panel_plot <- get("p_vol_pb_sc")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S01/panel_C.pdf"
  output_svg <- "results/figure_S01/panel_C.svg"
}

plot_figure_s01_c(output_pdf = output_pdf, output_svg = output_svg)
