plot_figure_s05_a <- function(output_pdf, output_svg) {
  ...
  df <- bind_rows(df_bulk, df_pseudo) %>% mutate(Numeric_Depth = as.numeric(gsub("^1_|\\.0$", "", Depth_Label)), path = paste0("/Volumes/Backup/20251120/evaluate_performance_real/", tolower(Dataset_Type), "/", tolower(Technology), "/", Depth_Label, "/", Replicate, ".stats"))
  ...
  depth_discovery <- ggplot(df_summary, aes(x = Numeric_Depth, y = Mean_F1, color = Dataset_Type, fill = Dataset_Type)) + geom_ribbon(aes(ymin = Mean_F1 - SD_F1, ymax = Mean_F1 + SD_F1), alpha = 0.1, color = NA) + geom_line(linewidth = 1.5) + geom_point(size = 3) + facet_wrap(~Technology) + scale_x_continuous(labels = label_number(scale = 1e-06, suffix = "M"), breaks = pretty_breaks(n = 5)) + scale_color_manual(values = c(Bulk = "#E7B800", Pseudobulk = "#2E9FDF")) + scale_fill_manual(values = c(Bulk = "#E7B800", 
      Pseudobulk = "#2E9FDF")) + theme_bw(base_size = 16) + labs(title = "Depth equivalency: transcript discovery (GENCODE)", y = "F1 Score (Intron Chain)", x = "Sequencing depth (million reads)", color = "Dataset", fill = "Dataset") + theme(legend.position = "bottom", strip.background = element_rect(fill = "gray95"), strip.text = element_text(face = "bold"))
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S05/panel_A.pdf"
  output_svg <- "results/figure_S05/panel_A.svg"
}

plot_figure_s05_a(output_pdf = output_pdf, output_svg = output_svg)
