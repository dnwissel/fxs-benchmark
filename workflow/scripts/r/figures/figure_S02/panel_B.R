plot_figure_s02_b <- function(output_pdf, output_svg) {
  ...
  length_bias_bulk_pb <- combined_frame %>% left_join(transcript_annotation_frame) %>% filter(pb_bulk_detection != "Other") %>% ggplot(aes(x = pb_bulk_detection, y = transcript_length)) + geom_violin() + theme_big_simple() + facet_wrap(~facet_label, nrow = 1) + scale_y_log10() + labs(y = "Length (nts)", x = "Detected by", title = "PB (Bulk) misses due to bias against short transcripts")

  if (!exists("length_bias_bulk_pb")) {
    stop("Panel object not found: length_bias_bulk_pb", call. = FALSE)
  }

  panel_plot <- get("length_bias_bulk_pb")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S02/panel_B.pdf"
  output_svg <- "results/figure_S02/panel_B.svg"
}

plot_figure_s02_b(output_pdf = output_pdf, output_svg = output_svg)
