plot_figure_s02_g <- function(output_pdf, output_svg) {
  ...
  pb_pseudobulk_unique <- combined_frame %>% left_join(transcript_annotation_frame) %>% filter(pb_exclusive_pseudobulk_detection != "Other") %>% ggplot(aes(x = pb_exclusive_pseudobulk_detection, y = coverage)) + geom_violin(scale = "width") + theme_big_simple() + facet_wrap(~facet_label, nrow = 1) + labs(y = "Gene coverage", x = "Detected by", title = "PB (SC) detections associated with truncations") + scale_y_log10()

  if (!exists("pb_pseudobulk_unique")) {
    stop("Panel object not found: pb_pseudobulk_unique", call. = FALSE)
  }

  panel_plot <- get("pb_pseudobulk_unique")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S02/panel_G.pdf"
  output_svg <- "results/figure_S02/panel_G.svg"
}

plot_figure_s02_g(output_pdf = output_pdf, output_svg = output_svg)
