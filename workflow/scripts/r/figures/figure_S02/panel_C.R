plot_figure_s02_c <- function(output_pdf, output_svg) {
  ...
  length_bias_bulk_ont <- combined_frame %>% left_join(transcript_annotation_frame) %>% filter(ont_bulk_detection != "Other") %>% ggplot(aes(x = ont_bulk_detection, y = transcript_length)) + geom_violin() + theme_big_simple() + facet_wrap(~facet_label, nrow = 1) + scale_y_log10() + labs(y = "Length (nts)", x = "Detected by", title = "ONT (Bulk) misses due to bias against long transcripts")

  if (!exists("length_bias_bulk_ont")) {
    stop("Panel object not found: length_bias_bulk_ont", call. = FALSE)
  }

  panel_plot <- get("length_bias_bulk_ont")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S02/panel_C.pdf"
  output_svg <- "results/figure_S02/panel_C.svg"
}

plot_figure_s02_c(output_pdf = output_pdf, output_svg = output_svg)
