plot_figure_s02_j <- function(output_pdf, output_svg) {
  ...
  coverage_difference_inf_var <- ggplot(plot_data, aes(x = transcript_illumina_mean_inf_rv, fill = set)) + geom_histogram(alpha = 0.5, position = "identity", bins = 50) + scale_x_log10() + scale_fill_manual(values = c(`Query higher` = "#e41a1c", `Query lower` = "#377eb8", Similar = "grey80")) + facet_wrap(~facet_label, nrow = 1) + theme_big_simple() + scale_y_continuous(trans = "pseudo_log") + labs(x = "Mean inferential relative variance (Illmn bulk)", y = "Count", fill = "Bias") + labs(title = "")

  if (!exists("coverage_difference_inf_var")) {
    stop("Panel object not found: coverage_difference_inf_var", call. = FALSE)
  }

  panel_plot <- get("coverage_difference_inf_var")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S02/panel_J.pdf"
  output_svg <- "results/figure_S02/panel_J.svg"
}

plot_figure_s02_j(output_pdf = output_pdf, output_svg = output_svg)
