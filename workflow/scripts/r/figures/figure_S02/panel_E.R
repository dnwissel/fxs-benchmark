plot_figure_s02_e <- function(output_pdf, output_svg) {
  ...
  pb_bulk_unique <- combined_frame %>% left_join(transcript_annotation_frame) %>% filter(pb_exclusive_bulk_detection != "Other" & !is.na(transcript_illumina_mean_inf_rv)) %>% ggplot(aes(x = pb_exclusive_bulk_detection, y = transcript_illumina_mean_inf_rv)) + facet_wrap(~facet_label, nrow = 1) + geom_violin(scale = "width") + theme_big_simple() + labs(y = "Inferential variance", x = "Detected by", title = "PB (Bulk) unique detections due to good resolution of inf. variance") + scale_y_log10()
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S02/panel_E.pdf"
  output_svg <- "results/figure_S02/panel_E.svg"
}

plot_figure_s02_e(output_pdf = output_pdf, output_svg = output_svg)
