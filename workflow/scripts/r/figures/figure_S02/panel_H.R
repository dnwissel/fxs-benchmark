plot_figure_s02_h <- function(output_pdf, output_svg) {
  ...
  ont_pseudobulk_unique <- combined_frame %>% left_join(transcript_annotation_frame) %>% filter(ont_exclusive_pseudobulk_detection != "Other") %>% ggplot(aes(x = ont_exclusive_pseudobulk_detection, y = coverage)) + geom_violin(scale = "width") + theme_big_simple() + facet_wrap(~facet_label, nrow = 1) + labs(y = "Gene coverage", x = "Detected by", title = "ONT (SC) detections associated with truncations") + scale_y_log10()

  if (!exists("ont_pseudobulk_unique")) {
    stop("Panel object not found: ont_pseudobulk_unique", call. = FALSE)
  }

  panel_plot <- get("ont_pseudobulk_unique")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S02/panel_H.pdf"
  output_svg <- "results/figure_S02/panel_H.svg"
}

plot_figure_s02_h(output_pdf = output_pdf, output_svg = output_svg)
