plot_figure_s02_d <- function(output_pdf, output_svg) {
  ...
  transcript_coverage_bias_pseudobulk <- combined_frame %>% left_join(transcript_annotation_frame) %>% filter(pseudobulk_detection != "Other") %>% ggplot(aes(x = pseudobulk_detection, y = coverage)) + geom_violin() + facet_wrap(~facet_label, nrow = 1) + theme_big_simple() + labs(x = "Detected by", y = "Gene coverage", title = "SC-specific detections associated with truncated reads")
...
if (exists("snakemake")) {
  ...
  output_pdf <- "results/figure_S02/panel_D.pdf"
  output_svg <- "results/figure_S02/panel_D.svg"
}

plot_figure_s02_d(output_pdf = output_pdf, output_svg = output_svg)
